...
import { getConsultionsData } from '../../store/reducers/consultions';
...
const Consultation = () => {
	...
	const defaultColumns = [
		...
		},
		{
			...
			key: document.body.getAttribute('dir') == 'ltr' && 'CON_TYPE_NAME_EN' ? 'CON_TYPE_NAME_EN' : 'CON_TYPE_NAME',
		},
		{
			label: document.body.getAttribute('dir') == 'ltr'  ? formatMessage({id: 'subjectEn'}) : formatMessage({id: 'subject'}),
			key: document.body.getAttribute('dir') == 'ltr' && 'CON_SUBJECT_EN' ? 'CON_SUBJECT_EN' : 'CON_SUBJECT',
		},
		{
			label: document.body.getAttribute('dir') == 'ltr'  ? formatMessage({id: 'briefEn'}) : formatMessage({id: 'brief'}),
			key: document.body.getAttribute('dir') == 'ltr' && 'CON_DESCRIPTION_EN' ? 'CON_DESCRIPTION_EN' : 'CON_DESCRIPTION',
		},
		{
	...
	useEffect(() => {
		dispatch(getConsultionsData({ theParams: { limit: limit, offset: limit * (page - 1) } })).then((res) => {
			// console.log(res.payload)
			const newArr = res?.payload?.data?.map((ele, i) => ({
				...ele,
				id: (i += 1),
				CON_DATE: new Date(ele.CON_DATE).toLocaleDateString(),
                CON_TIME_FROM: new Date(ele.CON_TIME_FROM).toISOString().split('T')[1].slice(0,5),
                CON_TIME_TO: new Date(ele.CON_TIME_TO).toISOString().split('T')[1].slice(0,5),
			}));
			// console.log(newArr);
			setData(newArr);
	...
	const handleChangeLimit = (dataKey) => {
		setPage(1);
		setLimit(dataKey);
		dispatch(getConsultionsData({ theParams: { limit: dataKey } })).then((res) => {
			const newArr = res?.payload?.data?.map((ele, i) => ({
				...ele,
				id: (i += 1),
				CON_DATE: new Date(ele.CON_DATE).toLocaleDateString(),
	...
	const handelSearchbtn = e => {
		if (e.key === 'Enter') {
			dispatch(getConsultionsData({ theParams: { limit: limit, offset: limit * (page - 1), searchVal: e.target.value  } })).then((res) => {
			const newArr = res?.payload?.data?.map((ele, i) => ({
				...ele,
				id: (i += 1),
				CON_DATE: new Date(ele.CON_DATE).toLocaleDateString(),
                CON_TIME_FROM: new Date(ele.CON_TIME_FROM).toLocaleDateString(),
                CON_TIME_TO: new Date(ele.CON_TIME_TO).toLocaleDateString(),
			}));
			console.log(newArr);
			setData(newArr);
            setPageTotal(res?.payload?.total)
