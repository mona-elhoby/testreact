...
const App = () => {
	...
	const defaultColumns = [
		...
		{
		  key: "INR_DATE",
		  label: formatMessage({id: 'informingDate'}),
		},
		{
		  key: document.body.getAttribute('dir') == 'ltr' && "INR_TYPE_NAME_EN" ? "INR_TYPE_NAME_EN" : "INR_TYPE_NAME",
		  label: formatMessage({id: 'informingType'}),
		},
		{
		  key: "INR_NUMBER",
		...
		{
		  key: document.body.getAttribute('dir') == 'ltr' && "INR_CATEGORY_NAME_EN" ? "INR_CATEGORY_NAME_EN" : "INR_CATEGORY_NAME",
		  label: formatMessage({id: 'informing'}),
		},
		{
			key: document.body.getAttribute('dir') == 'ltr' && 'INR_STATUS_NAME_EN' ? 'INR_STATUS_NAME_EN' : 'INR_STATUS_NAME',
	...
	const [data, setData] = React.useState(allInformings?.data);
	...
	useEffect(() => {
		dispatch(getInformingsData({ theParams: { limit: limit, offset: limit * (page - 1) } })).then((res) => {
			const newArr = res?.payload?.data?.map((ele, i) => ({
				...ele,
				id: (i += 1),
				INR_DATE: new Date(ele.INR_DATE).toLocaleDateString()
			}));
			console.log(newArr)
			setData(newArr);
		});
	...
	const handleChangeLimit = (dataKey) => {
		setPage(1);
		setLimit(dataKey);
		dispatch(getInformingsData({ theParams: { limit: dataKey} })).then((res) => {
			const newArr = res?.payload?.data?.map((ele, i) => ({
				...ele,
				id: (i += 1),
				INR_DATE: new Date(ele.INR_DATE).toLocaleDateString()
	...
	const handelSearchbtn = e => {
		if (e.key === 'Enter') {
			dispatch(getInformingsData({ theParams: { limit: limit, offset: limit * (page - 1), searchVal: e.target.value } })).then((res) => {
			const newArr = res?.payload?.data?.map((ele, i) => ({
				...ele,
				id: (i += 1),
				INR_DATE: new Date(ele.INR_DATE).toLocaleDateString()
